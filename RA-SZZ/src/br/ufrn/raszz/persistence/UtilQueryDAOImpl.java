...
public class UtilQueryDAOImpl extends UtilQueryDAO {
	...
	@Override
	public synchronized void insertDefects4jPatches(String project, String bug, String path, long linenumber,
			String content, boolean isRemovedLine) {			
		String sql = "INSERT INTO public.defects4jpatches "
				+ "(project, bug, path, linenumber, content, \"isRemovedLine\") "
				+ "values (:param1,:param2, :param3, :param4, :param5, :param6)";
		executeSQLWithParams(sql,project, bug, path, linenumber, content, isRemovedLine);
	}
	
	@Override
	public synchronized List<Object[]> getRemovedLineInDefects4jPatches(String project) {
		String sql = "select fixed_revision, bug, path, linenumber, substr(content,1) "
				+ "from defects4jpatches d inner join linkedissuegit l "
				+ "on (d.project = l.project and d.bug = l.bug_id) "
				+ "where d.project = :project and \"isRemovedLine\" = 't' "
				+ "and d.bug = '85'";
		SQLQuery query = currentSession.createSQLQuery(sql);
		query.setParameter("project", project);
		List<Object[]> result = query.list();
	...
	@Override
	public synchronized List<Object[]> getStartLinesForProject(String project, boolean validstatus, String szztable){
		String sql = "select distinct fixrevision, startpath, startlinenumber "
				+ "from "+ szztable + " where " // + "isvalidfix = :validstatus and "
						+ "project = :project";
		SQLQuery query = currentSession.createSQLQuery(sql);	
		List<Object[]> result = null;
		//query.setParameter("validstatus", validstatus);
		query.setParameter("project", project);
	...
	@Override
	public synchronized void updateHasInOtherImplementation(String project, String fixrevision, String startpath, long startlinenumber, boolean match, String szztable, String szz){ 
		String sql = "update " + szztable + " set hasMatch"+szz+ " = :param1 where " + 
			"project = :param2 and fixrevision = :param3 and " +
		    "startpath = :param4 and startlinenumber = :param5";
		List<Object> values = new ArrayList<Object>();
		executeSQLWithParams(sql, match, project, fixrevision, startpath, startlinenumber);
	}
